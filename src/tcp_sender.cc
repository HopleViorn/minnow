...
void TCPSender::push( const TransmitFunction& transmit )
{
	...
	//a variable equals to min(reader.bytes_buffered(), cwnd - sequence_numbers_in_flight() )
	uint64_t pseudo_cwnd = cwnd == 0? 1: cwnd;
	uint64_t max_size = min(reader.bytes_buffered(), pseudo_cwnd - sequence_numbers_in_flight());
	
	bool fin = reader.is_finished();
	zxc(fin);
	zxc(pseudo_cwnd - sequence_numbers_in_flight());
	if (reader.bytes_buffered() == 0){
		...
		if(pseudo_cwnd - sequence_numbers_in_flight() >=1 && fin){
			zxc("!");
			msg.FIN = true;
			bytes_in_flight++;
			next_seqno = next_seqno + 1;
	...
	while(bytes_sent < max_size){
		...
		uint64_t delta = min(head.size(), max_size - bytes_sent);
		if(delta == 0) break;
